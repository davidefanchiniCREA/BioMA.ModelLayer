namespace CRA.ModelLayer.Core
{
    ...
    public class RangeBasedCondition : ICondition
    {
        ...
        public string TestCondition(string callID)
        {
            ...
            {
                //DFa 18/4/2023: manage Precondition if NaN
                if (double.IsNaN((double)this._varInfo.CurrentValue))
                {
                    builderException.Append(this._varInfo.Name).Append(" cannot have a value of double.NaN ").Append(callID).Append(";\r\n");
                    throw new PreconditionErrorException(builderException.ToString());
                }
            ...
            if ((this._varInfo.ValueType == VarInfoValueTypes.ArrayDouble) || (this._varInfo.ValueType == VarInfoValueTypes.ListDouble))
            {
                ...
                {
                    //DFa 18/4/2023: manage Precondition if NaN
                    if (double.IsNaN(numArray[num])) {
                        builderException.Append(this._varInfo.Name).Append("[").Append(num.ToString()).Append("]").Append(" cannot be double.NaN ").Append(callID).Append(";\r\n");
                    }
                ...
                string preconditionsError = builderException.ToString();
                if (!preconditionsError.Equals(""))
                {
                    throw new PreconditionErrorException(preconditionsError);
                }
                return builder.ToString();
            }
            ...
            for (int i = 0; i < currentValue.GetLength(0); i++)
            {
                ...
                {
                    //DFa 18/4/2023: manage Precondition if NaN
                    if (double.IsNaN(currentValue[i, j]))
                    {
                        builderException.Append(this._varInfo.Name).Append("[").Append(i.ToString()).Append(",").Append(j.ToString()).Append("]").Append(" = ").Append(currentValue[i, j].ToString()).Append("] cannot be double.NaN ").Append(callID).Append(";\r\n");
                    }
            ...
            string preconditionError = builderException.ToString();
            if (!preconditionError.Equals(""))
            {
                throw new PreconditionErrorException(preconditionError);
            }
            return builder.ToString();
        }
