...
namespace CRA.ModelLayer.ParametersManagement
{
    ...
    public static class ParametersExtension
    {
        ...
        private static Dictionary<IParameters, KeyValuePair<IValuesReader, ReadOnlyCollection<VarInfo>>> _cachedVarInfoLists =
            new Dictionary<IParameters, KeyValuePair<IValuesReader, ReadOnlyCollection<VarInfo>>>(new ParametersClassComparer());

        private static Dictionary<IParameters, string> _cachedParameterKeyValues = new Dictionary<IParameters, string>(new ParametersClassComparer());

        // davide - prova - 17/11/2023 - begin

        private class ParametersClassComparer : IEqualityComparer<IParameters>
        {
            public bool Equals(IParameters x, IParameters y)
            {
                return x == y;
            }

            public int GetHashCode(IParameters obj)
            {
                return 0;
            }
        }

        // davide - prova - 17/11/2023 - end
        ...
        {
            if (parametersKey == null) throw new Exception("Cannot load paramaters for a null Parameters Key");
            if (parameterClass.Reader == null) throw new Exception("Cannot load parameters if the Reader is not configured.");
            parameterClass.Reader.LoadParameters(parametersKey, parameterClass);
        }
