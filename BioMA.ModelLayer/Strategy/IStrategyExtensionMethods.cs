namespace CRA.ModelLayer.Strategy
{
    ...
    public static class IStrategyExtensionMethods
    {
        ...
        {
            var associatedStrategiesTypeNames = strategy.ModellingOptionsManager.AllPossibleAssociatedStrategies;
            var associatedStrategies = new List<IStrategy>();
            if (associatedStrategiesTypeNames == null)
            {
            ...
            List<FieldInfo> fields =
                strategyType.GetFields(BindingFlags.NonPublic | BindingFlags.Instance)
                .Where(f => typeof(IStrategy).IsAssignableFrom(f.FieldType))
                .ToList();

            foreach (var strategyTypeName in associatedStrategiesTypeNames)
            {
                List<FieldInfo> fieldsWithType = fields
                    .Where(f => f.FieldType.FullName.Equals(strategyTypeName))
                    .ToList();
                foreach (FieldInfo fieldInfo in fieldsWithType)
                {
                    var associatedStrategy = (IStrategy)fieldInfo.GetValue(strategy);
                    if (associatedStrategy != null)
                    {
                        associatedStrategies.Add(associatedStrategy);
                    }
                }
            }
            return associatedStrategies;
        ...
        /// </summary>
        /// <param name="strategy">The strategy instance for which to discover associated strategies instances</param>
        /// <returns>The associated strategies instances</returns>
        public static List<IStrategy> GetAssociatedStrategies(this IStrategy strategy)
        {
            var associatedStrategiesTypeNames = strategy.ModellingOptionsManager.AssociatedStrategies;
            var associatedStrategies = new List<IStrategy>();
            if (associatedStrategiesTypeNames == null)
            {
                return associatedStrategies;
