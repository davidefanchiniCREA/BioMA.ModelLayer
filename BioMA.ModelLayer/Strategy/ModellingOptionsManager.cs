namespace CRA.ModelLayer.Strategy
{
    ...
    public class ModellingOptionsManager
    {
        ...
        public IEnumerable<string> AllPotentialAssociatedStrategies
        {
            get
            {
                ...
                foreach (KeyValuePair<string, Switch> pair in this.Switches)
                {
                    initialValues.Add(pair.Key, pair.Value.SwitchValue);
                }
                List<string> source = new List<string>();
                foreach (KeyValuePair<string, Switch> pair in this.Switches)
                {
                    foreach(string switchValue in pair.Value.AcceptableSwitchValues)
                    {
                        pair.Value.SwitchValue = switchValue;
                        source.AddRange(pair.Value.AssociatedStrategies);
                    }
                }
                // reput initial values
                foreach (KeyValuePair<string, Switch> pair in this.Switches)
                {
                    pair.Value.SwitchValue = initialValues[pair.Key];
                }
